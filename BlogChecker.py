...
def get_blog_api_url(blog_url):
    ...
    return "{0}/{1}/posts?per_page=20&orderby=date&order=desc".format(
        api_base, get_domain_only(no_protocol))
...
def parse_blogs(api_json):
    ...
    for post in api_json:
        bp = BlogPost()
        bp.publish_date = dateutil.parser.parse(post['date'],
                                                ignoretz=True).date()
...
for row in range(students_data.shape[0]):
    ...
    if student_number:
        ...
        records.append(rec)
        new_column.append(process_blog_colum(rec))
        print("Done")

now = datetime.datetime.now()

with open('output_{0}.html'.format(now.strftime("%Y_%m_%d_%H_%M_%S")),
          'w') as html_file:
